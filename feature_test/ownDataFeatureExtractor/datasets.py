...
if __name__ == '__main__':
    ...
    CD = ClassificationDataset("./dataset_output/", "dataset.json")
    TCD = TripletDataset("./dataset_output/", "dataset.json")

    for i in range(11):
        cid, a, p = TCD.__getitem__(i * 200)
        print(cid)

    """coco = COCO("./data/dataset_output/dataset.json") #COCO("./dataset_output/dataset.json")

    allAnn = []
    for Id in coco.getCatIds():
        annTemp = coco.getAnnIds(catIds=Id, iscrowd=0)
        allAnn.append(annTemp[0])
        print(len(annTemp))
